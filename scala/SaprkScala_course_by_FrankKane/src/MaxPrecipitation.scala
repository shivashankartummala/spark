...
object MaxPrecipitation {
   ...
   def parseLine(line:String)= {
    ...
    (stationID, entryType, temperature, day)
  }
  ...
  def main(args: Array[String]) {
    ...
    val lines = sc.textFile("../SparkScala_Data/1800.csv")
    ...
    for (result <- results.sorted) {
       ...
       val temp = result._2
       val formattedTemp = f"$temp%.2f Inches"
       println(s"$station maximum precipitation: $formattedTemp") 
    }
